...
public class freeze_sword_plus extends ArtifactMainWeaponFather {
    public freeze_sword_plus(Player player, Integer slot) {
        super(player, slot);
        ItemStack itemStack=ItemStack.of(Material.GOLDEN_SWORD);
        ItemMeta itemMeta=itemStack.getItemMeta();
        itemMeta.displayName(Component.text("加强寒冰之剑"));
        itemMeta.lore(List.of(Component.text("冰冻 II", TextColor.color(168,168,168))));
    ...
    public void OnFight(EntityDamageByEntityEvent event) throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        if(effectFather==null){
            effectFather=ArtifactEffectType.FREEZE.createEffect((Player) event.getEntity(),60,3,this.getPlayer());
        }
        else{
            if(effectFather.getAmplifier()<3){
                effectFather.setAmplifier(3);
                effectFather.setDuration(60);
                effectFather.setCausePlayer(this.getPlayer());
            }
            else if(effectFather.getAmplifier()==3){
                effectFather.setDuration(Math.max(effectFather.getDuration(),60));
                effectFather.setCausePlayer(this.getPlayer());
            }
        }
