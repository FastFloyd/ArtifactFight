...
public class wither_sword_plus_plus extends ArtifactMainWeaponFather {
    public wither_sword_plus_plus(Player player, Integer slot) {
        super(player, slot);
        ItemStack itemStack=ItemStack.of(Material.STONE_SWORD);
        ItemMeta itemMeta=itemStack.getItemMeta();
        itemMeta.displayName(Component.text("强化凋零之剑"));
        itemMeta.lore(List.of(Component.text("凋零 III", TextColor.color(168,168,168))));
    ...
    public void OnFight(EntityDamageByEntityEvent event) throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        if(effectFather==null){
            effectFather=ArtifactEffectType.WITHER.createEffect((Player) event.getEntity(),150,3,this.getPlayer());
        }
        else{
            if(effectFather.getAmplifier()<3){
                effectFather.setAmplifier(3);
                effectFather.setDuration(150);
                effectFather.setCausePlayer(this.getPlayer());
            }
            else if(effectFather.getAmplifier()==3){
                effectFather.setDuration(Math.max(effectFather.getDuration(),150));
                effectFather.setCausePlayer(this.getPlayer());
            }
        }
