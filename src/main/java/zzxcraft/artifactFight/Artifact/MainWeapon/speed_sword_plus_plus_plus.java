...
public class speed_sword_plus_plus_plus extends ArtifactMainWeaponFather {
    private int attack;
    private int fight;
    public speed_sword_plus_plus_plus(Player player, Integer slot) {
        super(player, slot);
        this.attack=0;
        this.fight=0;
        ItemStack itemStack=ItemStack.of(Material.WOODEN_SWORD);
        ItemMeta itemMeta=itemStack.getItemMeta();
        itemMeta.displayName(Component.text("超强化轻盈之剑"));
        itemMeta.lore(List.of(Component.text("轻盈 IV", TextColor.color(168,168,168))));
        ...
        itemMeta.addAttributeModifier(Attribute.ATTACK_SPEED,new AttributeModifier(new NamespacedKey(ArtifactFight.getMainClass(), UUID.randomUUID().toString()),0.4, AttributeModifier.Operation.ADD_NUMBER, EquipmentSlotGroup.MAINHAND));
    ...
    public void OnFight(EntityDamageByEntityEvent event) throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        event.setDamage(event.getDamage()+attack);
        this.fight=50;
        this.attack++;
    }
