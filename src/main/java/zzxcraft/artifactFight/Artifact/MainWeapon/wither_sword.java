...
public class wither_sword extends ArtifactMainWeaponFather {
    public wither_sword(Player player, Integer slot) {
        super(player, slot);
        ItemStack itemStack=ItemStack.of(Material.STONE_SWORD);
        ItemMeta itemMeta=itemStack.getItemMeta();
        itemMeta.displayName(Component.text("凋零之剑"));
        itemMeta.lore(List.of(Component.text("凋零 I", TextColor.color(168,168,168))));
    ...
    public void OnFight(EntityDamageByEntityEvent event) throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        if(effectFather==null){
            effectFather=ArtifactEffectType.WITHER.createEffect((Player) event.getEntity(),60,1,this.getPlayer());
        }
        else{
